...
export default function SearchBar({ onSearch }: SearchBarProps) {
  const [locationState, setLocation] = useState<string>("Lucknow, Uttar Pradesh");
  ...
  const { location, updateLocation, locationData } = useLocationStore();
  const [locationDataArray, setLocationDataArray] = useState(["Lucknow, Uttar Pradesh", "Gomti Nagar", "Indira Nagar", "Hazratganj", "Aliganj"]);
  const isMobile = useIsMobile();
  // Handle search submission
  const handleSearch = () => {
    const filter: any = {
      location: location !== "Lucknow, Uttar Pradesh" ? [locationState] : [],
  ...
  useEffect(() => {
    if (location) {
      setLocationDataArray(prevState => [...prevState, locationData]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                  <div className="col-span-1 lg:col-span-2">
                    ...
                    <div className="relative flex items-center gap-2">
                      <Select defaultValue={locationState} onValueChange={(e) => { setLocation(e) }} value={locationState}>
                        <SelectTrigger className="w-full">
                          <SelectValue placeholder="Select location" />
                        </SelectTrigger>
                        <SelectContent>
                          {locationDataArray && locationDataArray?.map((item: any, index: number) => {
